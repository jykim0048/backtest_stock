...
def add_detail_sheets(wb, b, d):
    ...
    # 2026-09-22 카드별 '시총비' 열 추가 → S1 A:E · S2 G:L · S3 N:S (구분 열 F·M)
    W_SL = {"A": 4.5, "B": 16.0, "C": 12.5, "D": 12.0, "E": 9.0, "F": 2.0,
            "G": 4.5, "H": 16.0, "I": 12.5, "J": 10.5, "K": 10.0, "L": 9.0, "M": 2.0,
            "N": 4.5, "O": 16.0, "P": 12.5, "Q": 10.5, "R": 10.0, "S": 9.0}
    ...
    if (slw.get("shortTop") or slw.get("loanUp") or slw.get("loanDown")):
        ...
        footnote("랭킹 유니버스(순매수 상위 30 등재 종목) 한정 · " + PL["loan"]
                 + " · 시총비=공매도 누적÷시가총액, 대차잔고÷시가총액("
                 + (slw.get("capBasis") or "KIS 마스터 시총") + ")")
        b.finish()
        ws2.freeze_panes = "A3"                       # 타이틀 바(기간) 고정
